...
int main(int argc, char *argv[])
{
	...
	while (getline(in, next_line)) { //What;s that in in getline
			istringstream iss(next_line); //WHAT'S A STRINGSTREAM?
			string token;
			while (iss >> token) {
					string nopunct = "";
	...
	// Enter vector values into a map 
  ...
  for (vector<string>::iterator it=tokens.begin(); it!=tokens.end(); it++) {
    wordmap[state].push_back(*it); //I don't understand this quite well
    state.push_back(*it);
    state.pop_front();
  }
